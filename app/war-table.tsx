    ...
    <main className="war-shell">
      ...
      <section className="workspace-grid">
        ...
        <section ref={mapRef} className={`map-panel tool-${tool}`} aria-label="헤이나펄 전장 작전판" onDragOver={(event) => event.preventDefault()} onDrop={handleMapDrop} onPointerDown={handleMapPointerDown} onPointerUp={handleMapPointerUp}>
          <div className="map-image-layer" /><div className="map-grid-lines" /><div className="faction lucia"><small>LUCIA</small><strong>BLUE FORCE</strong></div><div className="faction ian"><small>YAEN</small><strong>YELLOW FORCE</strong></div>
          <div className="objective life-stone"><span>◆</span><b>생명석</b></div><div className="objective hall hall-a"><span>♜</span><b>전당 A</b></div><div className="objective hall hall-b"><span>♜</span><b>전당 B</b></div><div className="objective hall hall-c"><span>♜</span><b>전당 C</b></div><div className="objective hall hall-d"><span>♜</span><b>전당 D</b></div><div className="objective tree courage-a"><span>♣</span><b>용기 영목</b></div><div className="objective tree courage-b"><span>♣</span><b>용기 영목</b></div>
          ...
          <div className="map-note"><span>MAP ASSET</span><strong>헤이나펄 원본 지도 대기 중</strong><small>/public/maps/heinapel-map.png 교체 준비 완료</small></div><div className="map-coordinates"><span>36° 14′ N</span><span>{TOOL_META.find((item) => item.id === tool)?.hint}</span><span>128° 03′ E</span></div>
        </section>
